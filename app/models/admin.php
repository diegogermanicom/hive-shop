<?php
    ...
    class Admin extends AdminModel {
        ...
        public function get_product($id_product) {
            ...
            if($result->num_rows != 0) {
                $row = $result->fetch_assoc();
                $row['price'] = $this->parse_float_price_back($row['price']);
                return $row;
            } else {
                return 'error';
            }
        ...
        public function get_languages($page = 1, $per_page = 20) {
            ...
            if($result->num_rows > 0) {
                ...
                $html .= '<tbody>';
                foreach($pager['result'] as $row) {
                    $active = ($row['active'] == 1) ? 'Active' : 'Disabled';
                    $html .= '<tr>';
        ...
        public function get_languages_array() {
            $sql = 'SELECT * FROM '.DDBB_PREFIX.'ct_languages WHERE active = 1 ORDER BY id_language';
            $result = $this->query($sql);
        ...
        public function get_admin_type_list($id_admin_type = null) {
            $sql = 'SELECT * FROM '.DDBB_PREFIX.'ct_admin_type ORDER BY id_admin_type';
            $result = $this->query($sql);
            $html = '';
            if($result->num_rows != 0) {
                while($row = $result->fetch_assoc()) {
