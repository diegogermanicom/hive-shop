<?php
    ...
    class AdminAjax extends AdminModel {
        ...
        public function save_new_product() {
            ...
            $_POST['price'] = $this->parse_float_price($_POST['price']);
            $sql = 'INSERT INTO '.DDBB_PREFIX.'products (id_product_view, price, alias, id_state) VALUES (?, ?, ?, ?)';
            $this->query($sql, array($_POST['id_view'], $_POST['price'], $_POST['alias'], $_POST['id_state']));
        ...
        public function save_edit_product() {
            ...
            // I save the product data
            $_POST['price'] = $this->parse_float_price($_POST['price']);
            $sql = 'UPDATE '.DDBB_PREFIX.'products SET id_product_view = ?, price = ?, alias = ?, id_state = ? WHERE id_product = ?';
            $this->query($sql, array($_POST['id_view'], $_POST['price'], $_POST['alias'], $_POST['id_state'], $_POST['id_product']));
            // Saving categories
        ...
        public function add_related() {
            ...
            if($result->num_rows != 0) {
                if(!empty($_POST['attributes'])) {
                    ...
                    while($row = $result->fetch_assoc()) {
                        ...
                        $sql = 'SELECT id_product_related FROM '.DDBB_PREFIX.'products_related_attributes WHERE id_product_related = ? AND ('.$sql_where.')';
            ...
            $_POST['price_change'] = $this->parse_float_price($_POST['price_change']);
            $sql = 'INSERT INTO '.DDBB_PREFIX.'products_related (id_product, stock, price_change, main, id_state) VALUES (?, ?, ?, ?, ?)';
            $this->query($sql, array($_POST['id_product'], $_POST['stock'], $_POST['price_change'], $main, $_POST['id_state']));
        ...
        public function save_new_attribute_value($id_attribute, $type, $value) {
            ...
            $sql = 'SELECT * FROM '.DDBB_PREFIX.'ct_languages WHERE active = 1';
            $result = $this->query($sql);
            if($result->num_rows != 0) {
                while($row = $result->fetch_assoc()) {
